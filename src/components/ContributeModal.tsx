...
export default function ContributeModal({ isOpen, onClose }: { isOpen: boolean, onClose: () => void }) {
      ...
      <div className="bg-[#0A0A0A] border border-white/10 w-full max-w-4xl rounded-[3rem] overflow-hidden flex flex-col md:flex-row shadow-2xl overflow-y-auto max-h-[90vh]">
        ...
        <form onSubmit={handleSubmit} className="w-full md:w-1/2 p-10 space-y-6">
          ...
          <button 
            ...
            type="submit" 
            className={`w-full py-4 rounded-2xl font-black text-white tracking-widest transition-all ${
              isSubmitting || !file ? 'bg-slate-800 text-slate-500 cursor-not-allowed' : 'bg-gradient-to-r from-blue-600 to-indigo-600 hover:shadow-[0_0_30px_rgba(37,99,235,0.4)] hover:scale-[1.02]'
            }`}
          >
            {isSubmitting ? '正在通过 GITHUB API 提交 PR...' : '立即提交 (SUBMIT TO GITHUB)'}
          </button>

          <p className="text-[9px] text-slate-600 text-center leading-relaxed font-medium">
            点击提交后，系统将自动在 GitHub 创建 PULL REQUEST。<br/>
            管理员（也就是你）审阅通过后即可在主页显示。
          </p>
        </form>
      </div>
    </div>
