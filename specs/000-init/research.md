...
**Rationale**: 这样可以与后续 `001-optimize-layout-remove-seed` 的“提高显示密度”形成清晰演进关系。  
**Alternatives considered**:
- 直接把初始版本定义为 4-5 列：会与 `001` 规格重叠，削弱增量边界。

## 决策 3：保留 Seed 作为初始元数据的一部分

**Decision**: 在详情展示、投稿录入与同步后的数据模型中保留 `Seed` 字段。  
**Rationale**: 用户明确要求的是后续在 `001` 中移除，因此 `000-init` 需要反映其被保留的初始状态。  
**Alternatives considered**:
- 在基线中直接去除 `Seed`：会导致 `001` 缺少明确的变更对象。

## 决策 4：网页投稿直接生成 GitHub Pull Request
