...
本计划定义 Prompt Gallery 在 `001` 之前的初始产品基线：一个以高视觉风格为核心的静态画廊站点，支持浏览、搜索、分类筛选、详情复制、网页投稿与 Markdown/JSON 数据同步。计划刻意保留 `Seed` 字段与三列桌面布局，以作为后续 `001`、`002`、`003`、`004` 的演进起点。
...
详见 [research.md](research.md)。本阶段确认初始基线需要保留 `Seed`、桌面三列布局、文件系统驱动的数据模型，以及 GitHub App PR 投稿方式。
