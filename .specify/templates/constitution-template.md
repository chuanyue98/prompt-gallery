...
<!-- 示例：每个功能都作为独立的库开始；库必须自包含、可独立测试、有文档记录；需要明确的目的 - 不允许仅用于组织的库 -->
...
<!-- 示例：TDD 是强制性的：编写测试 → 用户批准 → 测试失败 → 然后实施；严格执行 红-绿-重构 循环 -->
...
<!-- 示例：文本 I/O 确保可调试性；需要结构化日志；或者：MAJOR.MINOR.BUILD 格式；或者：保持简单，遵循 YAGNI 原则 -->
...
## 治理
<!-- 示例：章程优先于所有其他实践；修订需要记录、批准、迁移计划 -->

[GOVERNANCE_RULES]
<!-- 示例：所有 PR/评审必须验证合规性；复杂度必须有正当理由；使用 [GUIDANCE_FILE] 进行运行时开发指导 -->
